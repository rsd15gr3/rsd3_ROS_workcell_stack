...
void CommunicatingSocket::send(const std::string msg) throw(SocketException)
{
    if(::send(sockDesc, (raw_type *) msg.c_str(), msg.length(), 0) < 0)
        throw SocketException("Send failed (send())", true);
}

std::string CommunicatingSocket::recv(int bufferLen) throw(SocketException)
{
    char buffer[bufferLen];
    if(::recv(sockDesc, (raw_type *) buffer, bufferLen, 0) < 1)
    {
